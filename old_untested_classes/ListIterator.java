...
// LISTING 15-7 Java’s interface java.util.ListIterator
public interface ListIterator<T> extends Iterator<T> {
     ...
     * iterator's remove() or add() method has not been called since then. That
     * is, you can call remove only once per call to next() or previous(). The
     * list has not been altered during the iteration except by calls to the
     * iterator's remove(), add(), or set() methods.
     *
     * @throws IllegalStateException if next() or previous() has not been
     * called, or if remove() or add() has been called already after the last
...
// duplicated here for reference and to show new behavior for remove.
    /**
     * Detects whether this iterator has gone before the first entry in the lis
     t.
     * 
     * 
